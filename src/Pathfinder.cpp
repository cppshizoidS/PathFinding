...
#include "../include/Fixed Clock.hpp"
...
int Pathfinder::calculateDistance(const Node& n1, const Node& n2) {
...
int Pathfinder::getIndex(const int X, const int Y) {
	return Y * SIZE_X + X;
}
...
bool Pathfinder::startPathfinding(std::vector<Node>& nodes, float& pathfind_time, bool diagonal, Node* start_node, Node* end_node) {
	...
	while (open_set.size() > 0) {
		...
		for (auto* n : this->getNeighbourNodes(nodes, node, diagonal)) {
			...
			const int tentative_g_cost = node->g_cost + Pathfinder::calculateDistance(*node, *n);
			if (tentative_g_cost < n->g_cost || !open_set.contains(*n)) {
				if (*n == *end_node) {
					pathfind_time = clock.getElapsedTime();
					Pathfinder::reconstructPath(node, start_node);
					return true;
				}
				...
				n->h_cost = Pathfinder::calculateDistance(*n, *end_node);
